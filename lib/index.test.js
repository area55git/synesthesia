...
it('converts an image to text to an image lossless', () => {
  const fixture = path.join(__dirname, '..', 'test', 'fixtures', 'image-text-image');
  const input = path.join(fixture, 'input');
  const output = path.join(fixture, 'output');

  const inputImagePath = path.join(input, 'color.png');
  const inputImageBuffer = fs.readFileSync(inputImagePath);
  ...
  return fromImage(inputImageBuffer)
    ...
    .then(toImage)
    .then(outputImageBuffer => {
      fs.writeFileSync(path.join(output, 'color.png'), outputImageBuffer);
      expect(() => equal(output, input)).not.toThrow();
    });
});
...
it('converts a wav to a wav lossless', () => {
  ...
  const samples = [0, -2147483648, 2147483647, 4];
...
it('throws an error for pngs without transparency', () => {
  const fixture = path.join(__dirname, '..', 'test', 'fixtures', 'no-alpha');
  const input = path.join(fixture, 'input');

  const inputImagePath = path.join(input, 'no-alpha.png');
  const inputImageBuffer = fs.readFileSync(inputImagePath);

  expect.assertions(2);

  return fromImage(inputImageBuffer).catch(error => {
    expect(error).toBeInstanceOf(Error);
    expect(error.message).toMatchSnapshot();
  });
});
