...
it('converts a jpg to wav', () => {
  ...
  return toWav(imageBuffer, 'image/jpeg').then(waveBuffer => {
    fs.writeFileSync(path.join(fixture, 'output', 'output.wav'), waveBuffer);
...
it('converts a wav to png', () => {
  ...
  const output = path.join(fixture, 'output');
  const waveBuffer = fs.readFileSync(path.join(fixture, 'input', 'input.wav'));

  expect.assertions(1);
  rimraf.sync(output);
  fs.mkdirSync(output);

  return toImage(waveBuffer, 10, 10).then(imageBuffer => {
