...
const { imageToWav, wavToImage } = require('./convert');
...
it('converts a wav to jpg correctly', () => {
  ...
  return wavToImage(wav.toBuffer()).then(imageBuffer => {
...
it('converts a jpg to wav correctly', () => {
  ...
  return imageToWav(imageBuffer).then(wavBuffer => {
    const wav = new WaveFile(wavBuffer);
    const availableSamples = wav.samples.length;

    expect(availableSamples).toEqual(9);
    expect(wav.samples).toEqual(expectedSamples);
  });
});
...
it('converts a wav to png correctly', () => {
  ...
  return wavToImage(wav.toBuffer(), { outputType: 'png' }).then(imageBuffer => {
...
it('converts a png to wav correctly', () => {
  ...
  return imageToWav(imageBuffer, { inputType: 'image/png' }).then(wavBuffer => {
    const wav = new WaveFile(wavBuffer);
    const availableSamples = wav.samples.length;
...
it('respects width and height options', () => {
  ...
  return wavToImage(wav.toBuffer(), { width: 1, height: 4 }).then(imageBuffer => {
    fs.writeFileSync(path.join(fixture, 'output', 'output.jpg'), imageBuffer);
    expect(() => equal(output, expected)).not.toThrow();
  });
});
...
it('infers width correctly when only height is set', () => {
  ...
  return wavToImage(wav.toBuffer(), { height: 4 }).then(imageBuffer => {
    fs.writeFileSync(path.join(fixture, 'output', 'output.jpg'), imageBuffer);
    expect(() => equal(output, expected)).not.toThrow();
  });
});
...
it('infers height correctly when only width is set', () => {
  ...
  return wavToImage(wav.toBuffer(), { width: 1 }).then(imageBuffer => {
    fs.writeFileSync(path.join(fixture, 'output', 'output.jpg'), imageBuffer);
    expect(() => equal(output, expected)).not.toThrow();
  });
});
...
it('allows you to set a sample rate', () => {
  ...
  return imageToWav(imageBuffer, { sampleRate: 8000 }).then(wavBuffer => {
    const wav = new WaveFile(wavBuffer);
